...
def validate_source_adapter_contract(
    adapter: object, module: ModuleType
) -> SourceAdapter:
    ...
    if AdapterCapability.OUTPUT_RENDER in manifest.capabilities:
        raise ValueError(
            f"{module.__name__} adapter {manifest.adapter_id} cannot declare output render capability"
        )
    required_methods = (
        "match",
    ...
    if not all(has_callable(adapter, method_name) for method_name in required_methods):
        raise TypeError(
            f"{module.__name__} ADAPTER does not implement the source adapter contract"
        )
    return cast(SourceAdapter, adapter)


def validate_output_adapter_contract(
    adapter: object, module: ModuleType
) -> OutputAdapter:
    manifest = validated_manifest(manifest_from_adapter(adapter, module))
    ...
    if not has_callable(adapter, "render"):
        raise TypeError(
            f"{module.__name__} ADAPTER does not implement the output adapter contract"
        )
...
def manifest_from_adapter(adapter: object, module: ModuleType) -> AdapterManifest:
    ...
    if (
        manifest is None
        or not hasattr(manifest, "adapter_id")
        or not hasattr(manifest, "capabilities")
    ):
