...
from tallylot.application.resource_refs import to_workspace_path
...
def workspace_init(
    ...
    response = initialize_workspace_use_case().execute(
        WorkspaceInitRequest(workspace_root_ref=to_workspace_path(resolved_root))
    )
    emit_response(
        {
            "workspace_root_ref": response.workspace_root_ref,
