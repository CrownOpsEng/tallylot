...
def normalize_deposit_rows(profile: SourceProfile, path: Path) -> list[NormalizedTransaction]:
...
def normalize_withdraw_rows(profile: SourceProfile, path: Path) -> list[NormalizedTransaction]:
    events: list[NormalizedTransaction] = []
    for index, row in enumerate(read_rows(path), start=2):
        if (row.get("Status") or "").strip().lower() != "completed":
            continue
