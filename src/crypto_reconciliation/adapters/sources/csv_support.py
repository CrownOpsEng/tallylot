"""Shared CSV traversal support for source adapters."""
...
type CsvRowParseResult = NormalizedTransaction | IssueRecord | None
...
def collect_row_normalization(
    raw_dir: Path,
    parse_row: CsvRowParser,
    *,
    pattern: str = "*.csv",
    skip_file: Callable[[Path], bool] | None = None,
) -> tuple[tuple[NormalizedTransaction, ...], tuple[IssueRecord, ...]]:
    ...
    return tuple(transactions), tuple(issues)
