...
def _inventory_file_details(path: Path) -> tuple[tuple[str, ...], int | None, _TimezoneDetails]:
    ...
    header, rows = _inventory_csv_content(path)
    row_count = len(rows)
    if not header:
...
def _csv_timezone_details(
    ...
    timestamp_field = next((name for name in header if _is_timestamp_field(name)), "")
    ...
    source_timezone = _filename_timezone(filename)
    timezone_mode = ""
    timezone_value = ""
    timezone_conflict = ""
    ...
    if header_utc and _value_has_non_utc_offset(sample_value):
        ...
        timezone_value = _format_timezone_value(source_timezone)
    ...
    parsed_values = [
        parsed
        for value in values
        if (parsed := _parse_inventory_timestamp(value, source_timezone=source_timezone)) is not None
    ]
    parsed_values.sort()
...
def _inventory_csv_content(path: Path) -> tuple[tuple[str, ...], list[dict[str, str]]]:
    ...
    filtered_rows = [row for row in content_rows if not _is_placeholder_no_data_row(row)]
...
def _is_placeholder_no_data_row(row: Mapping[str, str | list[str]]) -> bool:
...
def _filename_timezone(filename: str) -> timezone | None:
...
def _format_timezone_value(value: timezone) -> str:
...
def _parse_inventory_timestamp(value: str, *, source_timezone: timezone | None) -> datetime | None:
    text = value.strip()
    if not text:
        return None
...
def _exact_inventory_timestamp(text: str) -> datetime | None:
    ...
    if _value_has_non_utc_offset(text):
