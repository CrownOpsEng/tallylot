...
from .screening_columns import (
    build_cointracking_column_map as _build_cointracking_column_map,
)
...
def screen_candidate(
    ...
    baseline_tx_ids = {
        row.get("Tx-ID", "") for row in baseline_rows if row.get("Tx-ID")
    }
    ...
    has_time_overlap = any(
        parse_timestamp(row["Date"]) <= baseline_cutoff for row in valid_rows
    )
    overlap_result = (
        None
        if issues
        else summarize_candidate_overlap(baseline_export_dir, candidate_path)
    )
