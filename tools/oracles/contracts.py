...
class ScreeningResult:
    ...
    def passed(self) -> bool:
        overlap_flagged = (
            False
            if self.overlap_result is None
            else bool(self.overlap_result.summary["rows_flagged"])
        )
        return (
            not self.issues
            and self.duplicate_count == 0
            and not self.has_time_overlap
            and not overlap_flagged
        )
    ...
    def blocked_reason_codes(self) -> tuple[str, ...]:
        ...
        if (
            self.overlap_result is not None
            and self.overlap_result.summary["rows_flagged"]
        ):
...
class BaselineArtifacts:
    asset_snapshot_rows: tuple[Mapping[str, str], ...]
    reconciliation_rows: tuple[Mapping[str, str], ...]
    negative_balances: tuple[Mapping[str, str], ...]
    source_activity_rows: tuple[Mapping[str, str], ...]
    cad_flow_rows: tuple[Mapping[str, str], ...]
    cad_balance_by_exchange_rows: tuple[Mapping[str, str], ...]
