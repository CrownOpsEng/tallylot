...
def scaffold_adapter(
    *,
    ...
    adapter_root = _adapter_root(spec.repo_root, spec.kind, spec.module_name)
    adapter_root.mkdir(parents=True, exist_ok=True)
    (adapter_root / "fixtures").mkdir(exist_ok=True)
    (adapter_root / "tests").mkdir(exist_ok=True)

    adapter_class_name = f"{_camel_case(spec.module_name)}{spec.kind.title()}Adapter"
    return (
        _write_file(
        ...
        _write_file(
            ...
            _contract_test_template(kind=spec.kind, module_name=spec.module_name),
            force=force,
        ),
        _write_file(
...
def _adapter_root(repo_root: Path, kind: str, module_name: str) -> Path:
    namespace = "sources" if kind == "source" else "outputs"
    return repo_root / "src" / "crypto_reconciliation" / "adapters" / namespace / module_name
        ...
        dedent(
            ...
            class {adapter_class_name}:
                manifest = AdapterManifest(
                    adapter_id=AdapterId("{spec.module_name}"),
...
def _contract_test_template(*, kind: str, module_name: str) -> str:
    namespace = "sources" if kind == "source" else "outputs"
    return dedent(
        f"""
        from __future__ import annotations
        ...
        def test_manifest_adapter_id_matches_package_name() -> None:
            assert str(ADAPTER.manifest.adapter_id) == "{module_name}"
        """
    )
