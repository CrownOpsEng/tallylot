...
def test_required_verification_paths_accepts_prefixed_export_filenames(tmp_path: Path) -> None:
    export_dir = tmp_path / "verification"
    export_dir.mkdir()
    for filename in (
    ...
    assert resolved["validate_transactions"].name.startswith("2026-03-22 CoinTracking - Validate Transactions")
