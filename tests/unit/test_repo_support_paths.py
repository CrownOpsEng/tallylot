...
def test_repo_support_paths_default_to_repo_checkout() -> None:
    ...
    assert (
        repo_paths.adapter_packs_root() == root / "tests" / "fixtures" / "adapter_packs"
    )
...
def test_override_repo_root_rebinds_derived_paths(tmp_path: Path) -> None:
    with repo_paths.override_repo_root(tmp_path):
        ...
        assert (
            repo_paths.adapter_packs_root()
            == tmp_path.resolve() / "tests" / "fixtures" / "adapter_packs"
        )
        assert (
            repo_paths.relative_repo_path(tmp_path / "docs" / "README.md")
            == "docs/README.md"
        )
...
def test_display_repo_path_falls_back_for_non_repo_paths(tmp_path: Path) -> None:
    assert repo_paths.display_repo_path(tmp_path / "outside.txt") == str(
