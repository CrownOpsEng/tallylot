...
def test_apply_package_rules_merges_same_cycle_near_duplicates_and_supersedes_older_conflicts() -> (
    None
):
    items = [
        package_item(
    ...
    assert (
        by_path["202203291730-export/borrow.csv"].package_row_status
        == "package_merge_into_primary"
    )
    assert (
        by_path["202203291730-export/trades.csv"].package_row_status
        == "package_merge_superseded_skip"
    )
...
def test_apply_package_rules_merges_multiple_same_cycle_members_into_one_primary() -> (
    None
):
    items = [
        package_item(
    ...
    assert (
        by_bundle["202203291930-export"].package_related_bundles
        == "202203291730-export; 202203291830-export"
    )
...
def test_apply_package_rules_requires_strictly_newer_marker_to_supersede_conflicts() -> (
    None
):
    items = [
        package_item(
