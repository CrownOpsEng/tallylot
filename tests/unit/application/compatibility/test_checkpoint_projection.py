...
def test_checkpoint_projection_uses_only_referenced_states_for_support_details() -> (
    None
):
    subject_ref = (
        "position",
    ...
    as_of = datetime(2026, 3, 22, 23, 59, 59, tzinfo=UTC)
    accepted_value = QuantityValue(quantity=Decimal("1.25"), subject_ref=subject_ref)
    selected_state = ReconciliationState(
        reconciliation_state_id="state-1",
        economic_facts_ref="facts-1",
        continuity_segment_records=(
            ContinuitySegmentRecord(
        ...
        checkpoint_proposal_records=(
            CheckpointProposalRecord(
                ...
                superseding_proposal_ref="",
                target_refs=("target-1",),
                evidence_refs=("selected-row",),
            ),
        ),
    )
    unrelated_state = ReconciliationState(
        reconciliation_state_id="state-2",
        economic_facts_ref="facts-2",
        continuity_segment_records=selected_state.continuity_segment_records,
        event_link_records=(),
        balance_target_records=selected_state.balance_target_records,
        checkpoint_proposal_records=(
            CheckpointProposalRecord(
    ...
    references = project_balance_references_from_checkpoint(
        checkpoint=checkpoint,
        reconciliation_states=(selected_state, unrelated_state),
        observation_details={
            "selected-row": ObservationCompatibilityDetail(
                support_ref="selected.pdf#page=1"
            ),
            "unrelated-row": ObservationCompatibilityDetail(
                support_ref="unrelated.pdf#page=1"
            ),
        },
    )

    assert references[0].support_ref == "selected.pdf#page=1"
