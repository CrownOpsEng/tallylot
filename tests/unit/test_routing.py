...
def test_resolve_routing_decision_reloads_inventory_after_file_changes(tmp_path: Path) -> None:
    repo_root = tmp_path / "repo"
    incoming_root = repo_root / "01_raw_exports" / "incoming"
    path = incoming_root / "account-export.csv"
    path.parent.mkdir(parents=True)
    ...
    inventory_header = (
        "source,activity_after_cutoff,first_post_cutoff_tx,export_window_start,export_window_end,import_order,status,"
        "capture_path,profile_status,adapter,normalization_status,exception_count,candidate_path,notes\n"
    )
    evidence_header = (
        "source,raw_dir,wallet_id,identifier_kind,normalized_identifier,display_identifier,network_scope,controller,"
        "account_label,evidence_kind,evidence_path,confidence,note\n"
    )
    inspection_row = {
        "family": "explorer_token_transfer_csv",
        "header_preview": "Transaction Hash | Blockno | UnixTimestamp | DateTime (UTC) | TokenValue | TokenSymbol | From | To",
    ...
    source_inventory.write_text(
        inventory_header
        + "source-a,yes,,2023-08-05 08:34:05,2025-12-31 23:59:59,1,capture_complete,"
        "01_raw_exports/source/source-a/2026-03,profiled,evm_explorer,ready,0,,\n",
        encoding="utf-8",
    )
    wallet_evidence.write_text(
        evidence_header
        + "source-a,/tmp/capture,evm_address:0x1111111111111111111111111111111111111111,evm_address,"
        "0x1111111111111111111111111111111111111111,0x1111111111111111111111111111111111111111,"
        "ethereum,Explorer export,Account 1,filename,/tmp/a.csv,high,\n",
        encoding="utf-8",
    )

    first = routing.resolve_routing_decision(
        repo_root=repo_root,
        incoming_root=incoming_root,
        path=path,
    ...
    source_inventory.write_text(
        inventory_header
        + "source-b,yes,,2023-08-05 08:34:05,2025-12-31 23:59:59,1,capture_complete,"
        "01_raw_exports/source/source-b/2026-03,profiled,evm_explorer,ready,0,,\n",
        encoding="utf-8",
    )
    wallet_evidence.write_text(
        evidence_header
        + "source-b,/tmp/capture,evm_address:0x1111111111111111111111111111111111111111,evm_address,"
        "0x1111111111111111111111111111111111111111,0x1111111111111111111111111111111111111111,"
        "ethereum,Explorer export,Account 2,filename,/tmp/b.csv,high,\n",
        encoding="utf-8",
    )

    second = routing.resolve_routing_decision(
        repo_root=repo_root,
        incoming_root=incoming_root,
        path=path,
        inspection_row=inspection_row,
