...
| ---- | ----: | ----: |
| Raw test definitions | 243 | 307 |
| Collected pytest tests | n/a | 355 |
...
| `tests/pipeline/test_intake_sort.py` | 17 | `tests/unit/application/services/intake/test_apply_workflow.py`; `tests/unit/application/services/intake/test_plan_package_review_workflow.py`; `tests/unit/application/services/intake/test_plan_routing_workflow.py`; `tests/unit/application/services/intake/packages/test_cycles.py`; `tests/unit/application/services/intake/packages/test_duplicates.py`; `tests/unit/application/services/intake/packages/test_isolation.py`; `tests/unit/application/services/intake/packages/test_merges.py`; `tests/unit/application/services/intake/packages/test_scope_conflicts.py`; `tests/unit/application/services/intake/packages/test_scope_precedence.py` | `ported-direct` | Legacy sort and grouping rules now live in typed intake planning and package-resolution seams under the dedicated intake package |
| `tests/unit/test_baseline_check.py` | 19 | `tests/unit/application/services/test_baseline_service.py`; `tests/unit/application/services/test_batch_staging_workflow.py`; `tests/contract/test_runtime_artifacts.py` | `ported-direct` | Baseline validation, artifact writing, exchange-only assets, and balance-only source handling are directly covered |
...
| `tests/unit/test_inspection.py` | 17 | `tests/unit/application/services/intake/test_archive_scan_workflow.py`; `tests/unit/application/services/intake/test_archive_scan_limits.py`; `tests/unit/application/services/intake/test_file_facts.py`; `tests/unit/application/services/test_profile_inventory_details.py`; `tests/unit/application/services/test_manifest_service.py` | `ported-direct` | Archive inspection, delimiter/date inference, artifact classification, and manifest/profile inventory behavior are direct seams |
...
| `tests/unit/test_package_resolution.py` | 23 | `tests/unit/application/services/intake/packages/test_cycles.py`; `tests/unit/application/services/intake/packages/test_duplicates.py`; `tests/unit/application/services/intake/packages/test_helpers.py`; `tests/unit/application/services/intake/packages/test_isolation.py`; `tests/unit/application/services/intake/packages/test_merges.py`; `tests/unit/application/services/intake/packages/test_scope_conflicts.py`; `tests/unit/application/services/intake/packages/test_scope_precedence.py`; `tests/unit/application/services/intake/test_plan_package_review_workflow.py` | `ported-direct` | Same-cycle merges, mixed-cycle review, scope conflicts, subset elimination, and overlap decisions are directly covered inside the intake feature package |
...
| `tests/unit/test_reconcile_source.py` | 3 | `tests/unit/application/services/test_supporting_services.py`; `tests/unit/application/services/test_verification_compare_workflow.py` | `ported-direct` | Reconciliation diffs and workflow outputs remain covered |
...
| `tests/unit/test_routing.py` | 12 | `tests/unit/application/services/intake/test_routing_portfolio.py`; `tests/unit/application/services/intake/test_routing_source_and_supporting.py`; `tests/unit/application/services/intake/test_inventory_service.py`; `tests/unit/application/services/intake/test_plan_routing_workflow.py` | `ported-direct` | Routing now includes capture inference, sidecar handling, supporting artifacts, and inventory-backed wallet routing inside the intake feature package |
...
| `tests/unit/test_source_manifest.py` | 8 | `tests/unit/application/services/intake/test_archive_scan_workflow.py`; `tests/unit/application/services/intake/test_archive_scan_limits.py`; `tests/unit/application/services/test_manifest_service.py`; `tests/contract/test_runtime_artifacts.py` | `ported-direct` | Source manifesting is archive-aware and relocation-safe in the typed workflow |
...
- Split the `structured_csv` adapter into package-style typed seams with direct
  ...
  module.
- Moved the intake workflow cluster into `application/services/intake/` and
  mirrored that layout under `tests/unit/application/services/intake/` so
  related seams stay grouped by feature instead of accumulating as flat
  same-prefix modules.
