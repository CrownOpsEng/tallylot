...
"""Screen a CoinTracking-ready import file for baseline overlap risk."""
...
def find_trade_table(export_dir: Path) -> Path:
    ...
    matches = sorted(
        ...
        if path.is_file() and TRADE_TABLE_MARKER in path.name and path.suffix.lower() == ".csv"
    )
    if not matches:
        raise FileNotFoundError(f"Missing required export containing {TRADE_TABLE_MARKER!r} in {export_dir}")
    if len(matches) > 1:
        match_names = ", ".join(path.name for path in matches)
        raise ValueError(f"Ambiguous export for {TRADE_TABLE_MARKER!r} in {export_dir}: {match_names}")
    return matches[0]
...
def summarize_overlap(
    ...
    baseline_dates = [parse_datetime(cell(row, baseline_columns["date"])) for row in baseline_rows if cell(row, baseline_columns["date"])]
...
def write_overlap_artifacts(out_dir: Path, summary: dict[str, object], flagged_rows: list[dict[str, str]]) -> None:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out_dir is not None:
        write_overlap_artifacts(args.out_dir.resolve(), summary, flagged_rows)
